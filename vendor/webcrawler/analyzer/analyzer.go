...
type Analyzer interface {
	Id() uint32 // 获得ID。
	Analyze(
		respParsers []ParseResponse, //分析函数列表
...
func (analyzer *myAnalyzer) Analyze(
	...
	if respParsers == nil {
		err := errors.New("The response parser list is invalid!///解析HTTP响应的函数列表为空")
		return nil, []error{err}
	}
	httpResp := resp.HttpResp()
	if httpResp == nil {
		err := errors.New("The http response is invalid!///这个http相应无效")
		return nil, []error{err}
	}
	var reqUrl *url.URL = httpResp.Request.URL
	logger.Infof("Parse the response ///解析响应(reqUrl=%s)... \n", reqUrl)
