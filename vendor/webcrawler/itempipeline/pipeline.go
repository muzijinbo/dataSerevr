...
func (ip *myItemPipeline) Send(item base.Item) []error {
	...
	index, ok1 := currentItem["index"].(int)
	sText, ok2 := currentItem["text"].(string)
	...
	/*	//fmt.Println(currentItem)
		//_ = createResult.WriteInfile("mydata1.txt", "写入文件")
		for k, v := range currentItem {
			toolcr.WriteStringInFile("mydata1.txt", "\r"+k)
			//var b []byte
			...
			toolcr.WriteBytesInFile("mydata1.txt", b)
			switch t := v.(type) {
			case string:
				toolcr.WriteStringInFile("mydata1.txt", t)

			case int:
				toolcr.WriteIntInFile("mydata1.txt", t)
			default:
				toolcr.WriteStringInFile("mydata1.txt", "unknow")
			}
			fmt.Println(currentItem)
		}*/
...
var summaryTemplate = "failFast///快速失败: %v, processorNumber: %d," +
